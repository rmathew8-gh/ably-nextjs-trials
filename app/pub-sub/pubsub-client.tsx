      ...
      <ChannelProvider channelName="status-updates">
        <div className="flex flex-row justify-center">
          <div className="flex flex-col justify-start items-start gap-10">
            <SampleHeader
              sampleName="Pub/Sub Channels"
              sampleIcon="PubSubChannels.svg"
              sampleDocsLink="https://ably.com/docs/getting-started/react#useChannel"
            />
            <div className="font-manrope text-base max-w-screen-sm text-slate-800 text-opacity-100 leading-6 font-light">
              ...
              Click&nbsp;
              <span className="font-medium">Publish from Server</span>&nbsp;to
              publish a message from a serverless function.
            </div>
            <PubSubMessages />
          </div>
        </div>
...
export function PubSubMessages() {
  const [logs, setLogs] = useState<Array<LogEntry>>([]);

  const { channel } = useChannel("status-updates", (message: Ably.Message) => {
    setLogs((prev) => [
      ...prev,
      new LogEntry(`✉️ event name: ${message.name} text: ${message.data.text}`),
    ]);
  });
